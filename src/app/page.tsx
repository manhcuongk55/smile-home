...
export default function DashboardPage() {
  ...
  function timeAgo(dateStr: string) {
    ...
    return `${Math.floor(hours / 24)}${t('d')} ${t('ago')}`;
  }
  ...
  return (
    <>
      <div className="page-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div>
          <h1>{t('dashboard')}</h1>
          <p>{t('welcomeBack')} — {t('overviewSubtitle')}</p>
        </div>
        <button className="btn btn-primary" onClick={handleSeed} disabled={seeding}>
          {seeding ? `⏳ ${t('seeding')}` : `🌱 ${t('seedDemoData')}`}
        </button>
      </div>
      ...
      <div className="stats-grid">
        ...
        <div className="stat-card emerald">
          ...
          <div className="stat-label">{t('roomOccupancy')}</div>
          <div className="stat-change positive">{occupancyRate}% {t('filled')}</div>
        </div>
      ...
      <div className="content-grid">
        ...
        <div>
          ...
          <div className="card" style={{ marginBottom: 16 }}>
            <h3 style={{ fontSize: '0.9rem', fontWeight: 600, marginBottom: 12 }}>{t('roomStatusBreakdown')}</h3>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{ fontSize: '0.8rem', color: 'var(--text-secondary)' }}>🟢 {t('vacant')}</span>
                <span style={{ fontWeight: 700 }}>{stats.vacantRooms}</span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{ fontSize: '0.8rem', color: 'var(--text-secondary)' }}>🔵 {t('occupied')}</span>
                <span style={{ fontWeight: 700 }}>{stats.occupiedRooms}</span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{ fontSize: '0.8rem', color: 'var(--text-secondary)' }}>🟡 {t('maintenance')}</span>
          ...
          <div className="card">
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              <a href="/interactions" className="btn btn-secondary btn-sm">➕ {t('newInteraction')}</a>
              <a href="/leads" className="btn btn-secondary btn-sm">🎯 {t('viewLeadPipeline')}</a>
              <a href="/rooms" className="btn btn-secondary btn-sm">🏠 {t('roomStatusBoard')}</a>
            </div>
          </div>
        </div>
      </div>
